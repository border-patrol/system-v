...
isFuncTy term with (view isFuncTy term)
  isFuncTy (Res u type term) | (HasView prf) with (type)
    isFuncTy (Res (IDX TYPE) type term) | (HasView Match) | (FuncTy a b)
      = Right (FTy a b term)
    isFuncTy (Res (IDX _) type term) | (HasView WrongTm) | ty'
      = Left (WrongType NotAType ty')
    isFuncTy (Res u type term) | (HasView NotATerm) | ty'
      = Left (WrongType NotAType ty')
...
data PortVect : {lvls  : Universes}
  ...
  where
    PV : {lvls  : Universes}
      ...
      -> {dir   : _}
      -> (w     : Whole)
      -> (term  : SystemV types (PortTy (VectorTyDesc w tyD) dir))
               -> PortVect types
...
isPortVect port with (view (isPortVect) port)
  isPortVect (Res u type term) | (HasView prf) with (type)
    isPortVect (Res (IDX TERM) type term) | (HasView Match) | (PortTy (VectorTyDesc w ty) dir)
      = Right (PV w term)
...
isPortAttr : {lvls  : Universes}
          ...
          -> (port  : Result TYPE SystemV lvls types)
          -> (d     : Direction)
                   -> TermBuilder (PortAttr types d)
isPortAttr port d
  = do (P dp ty p) <- isPort port
       let pg = PortTy ty dp
       let pe = PortTy ty d
...
public export
data PortTy : {lvls  : Universes}
           -> (types : Context lvls)
                    -> Type
  where
    PTy : (d     : TYPE (DATA TYPE))
       -> (dir   : Direction)
       -> (term  : SystemV types (PortTyDesc d dir))
                -> PortTy types
export
isPortTy : {lvls  : Universes}
        -> {types : Context lvls}
        -> (term  : Result TYPE SystemV lvls types)
                 -> TermBuilder (PortTy types)
isPortTy term with (view isPortTy term)
  isPortTy (Res u type term) | (HasView prf) with (type)
    isPortTy (Res (IDX TYPE) type term) | (HasView Match) | (PortTyDesc d dir)
      = Right (PTy d dir term)
    isPortTy (Res u type term) | (HasView Fail) | ty'
      = Left (WrongType NotAType ty')
...
data TypeFunc : {lvls    : Universes}
  ...
  where
    TF : {a : TYPE (IDX TYPE)}
      ...
      -> (p : SystemV types a)
      -> (r : SystemV types b)
      -> (p : Function.Type.Valid (IDX TYPE) (FuncTy a b))
           -> TypeFunc types

export
typeFunc : {lvls  : Universes}
        -> {types : Context lvls}
        -> (a     : Result TYPE SystemV lvls types)
        -> (b     : Result TYPE SystemV lvls types)
                 -> TermBuilder (TypeFunc types)
typeFunc a b
  = do (TT tyP param)  <- isType a
       (TT tyR return) <- isType b
       case Function.Type.valid (IDX TYPE) (FuncTy tyP tyR) of
         (Yes prfWhy) => Right (TF param return prfWhy)
         (No msgWhyNot prfWhyNot) =>
           Left (WrongType NotAType (FuncTy tyP tyR))
...
data Body : {lvls    : Universes}
  ...
  where
    B : {b    : TYPE (IDX TERM)}
     ...
     -> (chk  : Function.Term.Valid (IDX TERM) (FuncTy a b))
             -> Body types a
...
body a type
  = do (TTerm ty term) <- isTermTerm type
       case Function.Term.valid (IDX TERM) (FuncTy a ty) of
