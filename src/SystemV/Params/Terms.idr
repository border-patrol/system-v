...
data SystemV : Context lvls -> TYPE level -> Type where
  ...
  TyFuncD : {u : Universe}
         ...
         -> (paramTy  : SystemV ctxt         paramTYPE)
         -> (returnTy : SystemV ctxt                   returnTYPE)
         -> (vld      : Function.ValidType (IDX TYPE) (FuncDefTy u paramTYPE returnTYPE))
                     -> SystemV ctxt (FuncDefTy u paramTYPE returnTYPE)
  ...
  TyOverride : {typeInner, typeOuter : TYPE (DATA TYPE)}

            -> (inner : SystemV ctxt typeInner)
                     -> SystemV ctxt typeOuter
  ...
  FuncD : {u,v: Universe}
       ...
       -> (type : SystemV  ctxt    paramTyDesc)
       -> (def  : SystemV  ctxt    paramTy)
       -> (body : SystemV (ctxt +=             paramTy) bodyTy)

       -> (prf  : Default.TyCheck u v          paramTyDesc paramTy)
       -> (vld  : Function.ValidTerm (IDX TERM) (FuncDefTy v paramTy bodyTy))

              -> SystemV  ctxt        (FuncDefTy v paramTy  bodyTy)
  ...
  AppDef : {u       : Universe}
        ...
        -> (func    : SystemV ctxt (FuncDefTy u paramTy  bodyTy))
                   -> SystemV ctxt                       bodyTy
  ...
  IfThenElseC : (test  : SystemV ctxt BoolTy)
             -> (whenT : SystemV ctxt UnitTy)
             -> (whenF : SystemV ctxt UnitTy)
                      -> SystemV ctxt UnitTy
