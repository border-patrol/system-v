module SystemV.Annotated.Terms.NormalForm.Types
...
import SystemV.Annotated.Terms.NormalForm.Error
...
namespace TermType
  ...
  data NF : (term : SystemV ctxt type) -> Type where
    ...
    TyChan : DataType.NF type -> TermType.NF (TyChan type s i)
    TyPort : DataType.NF type -> TermType.NF (TyPort type dir s i)
...
namespace Chan
  public export
  data NF : (term : SystemV ctxt type) -> Type where
    MkChan : DataType.NF type -> Chan.NF (MkChan type s i)

namespace Gate
  public export
  data NF : (term : SystemV ctxt type) -> Type where
    Not : {output : SystemV ctxt (PortTy ty OUT s i)}
       -> Argument.NF output
       -> {input : SystemV ctxt (PortTy ty IN s i)}
       -> Argument.NF input
       -> Gate.NF (Not output input)

    Gate : {output : SystemV ctxt (PortTy ty OUT s i)}
        -> Argument.NF output
        -> {inputA,inputB : SystemV ctxt (PortTy ty IN s i)}
        -> Argument.NF inputA
        -> Argument.NF inputB
      ...
      namespace Sequence
        ...
        data NF : (term : SystemV ctxt type) -> Type where
          ...
          Connect : {left  : SystemV ctxt (PortTy ty dirL s i)}
                 ...
                 -> {right : SystemV ctxt (PortTy ty dirR s i)}
